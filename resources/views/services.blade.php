...
=======
            ...
            for(let hour = 10; hour <= 21; hour++) {
                for(let mins of ['00', '30']) {
                    ...
                    dates.forEach(d => {
                        ...
                        let isClosed = false;
                        if ([1, 2, 3, 4].includes(dayOfWeek) && hour >= 21) isClosed = true;
                        let isAvailable = (availableTimes[dateString] && availableTimes[dateString].includes(timeString));
                        ...
                        if (isClosed) {
                            ...
                            html += `<td class="p-2 border-l border-gray-100 text-center">
                                        <button type="button" onclick="selectSlot('${dateString}', '${timeString}', this)" class="w-full py-2 text-xs font-bold text-[#b5106a] bg-pink-50 hover:bg-[#b5106a] hover:text-white rounded transition-colors time-slot-btn">Select</button>
                                     </td>`;
                        }
                    });
                    html += `</tr>`;
    ...
    function selectSlot(date, time, btnElement) {
        ...
        let ampm = hour >= 12 ? 'PM' : 'AM';
        hour = hour % 12 || 12;
        document.getElementById('selected-slot-text').innerText = `${formattedDate} at ${hour}:${time.split(':')[1]} ${ampm}`;
        document.getElementById('input_appointment_date').value = selectedDate;
        document.getElementById('input_start_time').value = selectedTime;
        const servicesContainer = document.getElementById('hidden-services-inputs');
        servicesContainer.innerHTML = '';
        cart.forEach(item => { servicesContainer.innerHTML += `<input type="hidden" name="services[]" value="${item.id}">`; });
        const confirmBtn = document.getElementById('confirm-booking-btn');
        confirmBtn.disabled = false;
        confirmBtn.classList.remove('bg-gray-300', 'text-gray-500');
        confirmBtn.classList.add('bg-[#b5106a]', 'text-white', 'hover:bg-pink-700', 'shadow-lg');
